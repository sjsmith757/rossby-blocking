...
if __name__=="__main__":
    ...
    wavenum = int(sys.argv[2])
    cxpeak = float(sys.argv[3])
    fpeak = float(sys.argv[4])
    name = sys.argv[5]
    
    nwforce = 26
    nwcx = 21
    
    ffreqs = np.zeros((niter,nwforce))
    fspeeds = np.zeros((niter,nwforce))
    fphases = np.zeros((niter,nwforce))
    fampls = np.zeros((niter,nwforce))
    
    cfreqs = np.zeros((niter,nwcx))
    cspeeds = np.zeros((niter,nwcx))
    cphases = np.zeros((niter,nwcx))
    campls = np.zeros((niter,nwcx))
    
    nblockseq = np.zeros(niter)
    blockszseq = np.zeros(niter)
    ...
    for n in range(0,niter):
        conditions,nblocks,blocksize,uxs,uts,sct,sts,sxs = blocking_iteration(wavenum=wavenum,
                                                                              cxpeak=cxpeak,
                                                                              fpeak=fpeak,nwcx=nwcx,
                                                                              nwforce=nwforce)
